...
{name:"Whites",minutes:55,temp:"90°C"},{name:"Eco Wash",minutes:50,temp:"20°C"},
...
function Toast({message,type,onClose}){useEffect(()=>{const t=setTimeout(onClose,3500);return()=>clearTimeout(t)},[onClose]);const c={success:"#059669",error:"#dc2626",warning:"#d97706",info:"#0ea5e9"}[type]||"#0ea5e9";return<div style={{position:"fixed",top:10,right:10,left:10,zIndex:9999,padding:"11px 14px",background:c,color:"#fff",borderRadius:12,fontSize:12,fontWeight:600,boxShadow:`0 6px 24px ${c}44`,animation:"si .3s ease",maxWidth:340,margin:"0 auto"}}>{message}</div>;}

function Wash({on,prog,c,sz=130}){const i=sz-20;return<div style={{width:sz,height:sz,borderRadius:"50%",background:`conic-gradient(${c} ${prog*360}deg,#1a2236 ${prog*360}deg)`,display:"flex",alignItems:"center",justifyContent:"center",boxShadow:on?`0 0 40px ${c}33`:"none",animation:on?"rp 2s infinite":"none"}}><div style={{width:i,height:i,borderRadius:"50%",background:"#070b12",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column"}}>{on?<div style={{animation:"spin 2s linear infinite",fontSize:sz*.18}}>💧</div>:<span style={{fontSize:sz*.16,opacity:.35}}>⏻</span>}<div className="M" style={{fontSize:10,color:"#94a3b8",marginTop:3}}>{on?`${Math.round(prog*100)}%`:"IDLE"}</div></div></div>;}

function EspBadge({esp}){if(!esp)return<div className="P" style={{background:"#33415533",color:"#64748b"}}>ESP32 —</div>;const age=Date.now()/1000-(esp.lastSeen||0);const ok=age<90;return<div className="P" style={{background:ok?"#05966918":"#dc262618",color:ok?"#34d399":"#f87171"}}><span style={{width:5,height:5,borderRadius:"50%",background:"currentColor",animation:ok?"gl 2s infinite":"none"}}/>{ok?"Online":"Offline"}</div>;}

function Modal({children,onClose}){return<div style={{position:"fixed",inset:0,zIndex:9998,background:"#000b",display:"flex",alignItems:"center",justifyContent:"center",padding:10}} onClick={onClose}><div className="C" style={{width:"100%",maxWidth:370,padding:22,animation:"fu .25s ease",maxHeight:"88vh",overflowY:"auto",border:"1px solid #334155"}} onClick={e=>e.stopPropagation()}>{children}</div></div>;}

function Toggle({on,onChange,color}){return<div onClick={onChange} style={{width:36,height:18,borderRadius:9,background:on?color||"#0D9488":"#334155",cursor:"pointer",position:"relative",transition:"background .2s"}}><div style={{width:14,height:14,borderRadius:7,background:"#fff",position:"absolute",top:2,left:on?20:2,transition:"left .2s"}}/></div>;}
...
function Login({onLogin,cfg}){
  ...
  return<div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#070b12",padding:14}}>
    <div style={{width:"100%",maxWidth:340,animation:"fu .5s ease"}}>
      <div style={{textAlign:"center",marginBottom:22}}><div style={{fontSize:40,marginBottom:2}}>🫧</div><h1 style={{fontSize:24,fontWeight:800,letterSpacing:-.5,background:`linear-gradient(135deg,${cfg.primaryColor},${cfg.accentColor})`,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>{cfg.appName}</h1><p style={{color:"#475569",fontSize:11}}>Smart shared laundry</p></div>
      <div className="C" style={{padding:"22px 18px"}}>
        ...
        <input value={pw} onChange={e=>setPw(e.target.value)} type="password" placeholder={m==="admin"?"Admin password":"PIN"} onKeyDown={e=>e.key==="Enter"&&go()} className="I" style={{marginBottom:5}}/>
...
function ProfileMod({user,cfg,onClose,toast}){
  ...
  return<Modal onClose={()=>onClose(null)}>
    ...
    <div className="sb" style={{padding:"9px 10px",borderRadius:8,background:"#0c1220",marginBottom:12}}><div><div style={{fontSize:11,fontWeight:600}}>Do Not Disturb</div><div style={{fontSize:9,color:"#64748b"}}>Mute alerts</div></div><Toggle on={dnd} onChange={()=>sD(!dnd)} color={cfg.primaryColor}/></div>
...
function EditUserMod({u,cfg,onClose,toast}){
  ...
  return<Modal onClose={onClose}>
    <h3 style={{fontSize:16,fontWeight:700,marginBottom:12}}>Edit: {u.name}</h3>
    ...
    <div className="sb" style={{padding:"8px 10px",borderRadius:8,background:"#0c1220",marginBottom:6}}><span style={{fontSize:11}}>DND</span><Toggle on={dnd} onChange={()=>sD(!dnd)} color={cfg.primaryColor}/></div>
    <div className="sb" style={{padding:"8px 10px",borderRadius:8,background:dis?"#dc262612":"#0c1220",marginBottom:10}}><span style={{fontSize:11,color:dis?"#f87171":"#e2e8f0"}}>{dis?"Disabled":"Active"}</span><Toggle on={!dis} onChange={()=>sDis(!dis)} color="#059669"/></div>
...
// ═══════ ADMIN START FOR USER MODAL ═══════
function StartForUserMod({users,cfg,mac,onClose,toast}){
  const[sel,sS]=useState(null);const[cy,sCy]=useState(0);
  const go=async()=>{if(!sel)return toast("Pick a user","error");if(mac?.running)return toast("Machine busy","error");const c=cfg.washCycles[cy];await DB.setMachine({running:true,userId:sel.id,userName:sel.name,cycleName:c.name,startTime:Date.now(),durationMs:c.minutes*60000});toast(`Started ${c.name} for ${sel.name} (${c.minutes}m)`,"success");onClose()};
  return<Modal onClose={onClose}>
    <h3 style={{fontSize:16,fontWeight:700,marginBottom:12}}>Start wash for user</h3>
    <div style={{marginBottom:12}}>{users.filter(x=>!x.disabled).map(u=><button key={u.id} onClick={()=>sS(u)} className="B" style={{display:"flex",alignItems:"center",gap:6,width:"100%",marginBottom:4,background:sel?.id===u.id?`${cfg.primaryColor}22`:"#0c1220",border:sel?.id===u.id?`1.5px solid ${cfg.primaryColor}`:"1px solid #1e293b",textAlign:"left"}}><span>{u.emoji||"😊"}</span>{u.name}</button>)}</div>
    <div style={{fontSize:11,fontWeight:600,marginBottom:6}}>Cycle</div>
    <div style={{display:"flex",flexWrap:"wrap",gap:3,marginBottom:12}}>{cfg.washCycles.map((c,i)=><button key={i} onClick={()=>sCy(i)} className="B" style={{background:cy===i?`${cfg.primaryColor}18`:"#0c1220",color:"#e2e8f0",border:cy===i?`1.5px solid ${cfg.primaryColor}`:"1px solid #1e293b",fontSize:10,padding:"5px 8px"}}>{c.name} {c.minutes}m</button>)}</div>
    <div className="G2"><button onClick={onClose} className="B" style={{background:"#1e293b",color:"#94a3b8"}}>Cancel</button><button onClick={go} className="B" style={{background:cfg.primaryColor,color:"#fff"}}>Start</button></div>
  </Modal>;
}

// ═══════ HEADER ═══════
function Head({cfg,user,admin,onOut,onProf,esp}){return<div className="sb" style={{padding:"9px 12px",borderBottom:"1px solid #1e293b",flexWrap:"wrap",gap:5}}><div className="row"><span style={{fontSize:18}}>🫧</span><span style={{fontWeight:800,fontSize:15,letterSpacing:-.5}}>{admin?"Admin":cfg.appName}</span></div><div className="row" style={{gap:5}}><EspBadge esp={esp}/>{!admin&&<button onClick={onProf} style={{background:"none",border:"none",cursor:"pointer",fontSize:16,padding:1}}>{user.emoji||"😊"}</button>}<button onClick={onOut} className="B" style={{background:"#1e293b",color:"#64748b",padding:"3px 7px",fontSize:9}}>Out</button></div></div>;}
  ...
  return<div style={{minHeight:"100vh",background:"#070b12"}}>
    ...
    <div style={{maxWidth:600,margin:"0 auto",padding:"12px 10px"}}>
      ...
      {!mac?.running&&<div className="C" style={{marginBottom:12}}>
        ...
        <div style={{display:"flex",flexWrap:"wrap",gap:3,marginBottom:8}}>{cfg.washCycles.map((c,i)=><button key={i} onClick={()=>sCy(i)} className="B" style={{background:cy===i?`${cfg.primaryColor}18`:"#0c1220",color:"#e2e8f0",border:cy===i?`1.5px solid ${cfg.primaryColor}`:"1px solid #1e293b",fontSize:10,padding:"6px 9px"}}>{c.name} <span style={{color:"#64748b"}}>{c.minutes}m</span></button>)}</div>
        <div className="row" style={{padding:"7px 9px",borderRadius:7,background:"#0c1220",marginBottom:8,fontSize:11,color:"#94a3b8"}}>
          <span style={{fontWeight:600,color:"#e2e8f0"}}>{cfg.washCycles[cy].name}</span>
          <span>·</span><span>{cfg.washCycles[cy].minutes} min</span>
          <span>·</span><span>{cfg.washCycles[cy].temp}</span>
        </div>
        <button onClick={start} className="B" style={{width:"100%",padding:"11px 0",background:cfg.primaryColor,color:"#fff",fontSize:14,fontWeight:700}}>Start — {cfg.washCycles[cy].minutes} min</button>
      </div>}
      ...
      <div className="C"><span style={{fontSize:13,fontWeight:700}}>Housemates</span><div className="G2" style={{marginTop:8}}>{users.filter(x=>!x.disabled).map(u=><div key={u.id} className="row" style={{padding:"7px 8px",borderRadius:7,background:"#0c1220"}}><div className="A" style={{width:26,height:26,fontSize:13,background:u.id===mac?.userId?`${cfg.primaryColor}30`:"#1a2236"}}>{u.emoji||"😊"}</div><div><div style={{fontSize:10,fontWeight:600}}>{u.name}</div><div style={{fontSize:8,color:u.id===mac?.userId?"#34d399":u.dnd?"#f59e0b":"#475569"}}>{u.id===mac?.userId?"Washing":u.dnd?"DND":"Free"}</div></div></div>)}</div></div>
    </div>
  </div>;
}
...
function AdminDash({cfg,setCfg,onOut,toast}){
  ...
  return<div style={{minHeight:"100vh",background:"#070b12"}}>
    ...
    {sfu&&<StartForUserMod users={users} cfg={cfg} mac={mac} onClose={()=>sSFU(false)} toast={toast}/>}
    <div style={{maxWidth:660,margin:"0 auto",padding:"12px 10px"}}>
      ...
      {tab==="dash"&&<>
        <div className="G4" style={{marginBottom:12}}>
          <div className="S"><div style={{fontSize:9,color:"#64748b"}}>Users</div><div style={{fontSize:20,fontWeight:700}}>{users.filter(x=>!x.disabled).length}<span style={{fontSize:11,color:"#64748b"}}>/{users.length}</span></div></div>
        ...
        <div className="C" style={{marginBottom:12,display:"flex",alignItems:"center",gap:12}}>
          ...
          <div style={{flex:1}}>{mac?.running?<>
            ...
            <button onClick={()=>sSFU(true)} className="B" style={{marginTop:6,background:`${cfg.primaryColor}18`,color:cfg.primaryColor,fontSize:10,padding:"5px 12px",border:`1px solid ${cfg.primaryColor}33`}}>Start for user...</button>
          </>}</div>
        </div>

        <div className="C" style={{marginBottom:12}}>
          <span style={{fontSize:12,fontWeight:700}}>ESP32</span>
          ...
          </div>:<div style={{fontSize:11,color:"#475569",marginTop:6}}>No data — flash firmware</div>}
        </div>

        <div className="C" style={{marginBottom:12}}>
          <span style={{fontSize:12,fontWeight:700}}>Wash stats</span>
          <div style={{marginTop:8}}>{users.map(u=>{const cnt=washCount[u.name]||0;return<div key={u.id} className="sb" style={{padding:"5px 0"}}><div className="row"><span style={{fontSize:12}}>{u.emoji||"😊"}</span><span style={{fontSize:11}}>{u.name}</span>{u.disabled&&<span className="P" style={{background:"#dc262618",color:"#f87171"}}>off</span>}</div><div className="row" style={{gap:4}}><div style={{width:Math.min(cnt*14,120),height:5,borderRadius:3,background:cfg.primaryColor}}/><span className="M" style={{fontSize:10,color:"#94a3b8"}}>{cnt}</span></div></div>})}</div>
        </div>

        {sch.length>0&&<div className="C"><span style={{fontSize:12,fontWeight:700}}>Upcoming</span><div style={{marginTop:6}}>{sch.slice(0,5).map(s=><div key={s.id} style={{padding:"4px 0",fontSize:10,color:"#94a3b8"}}><strong style={{color:"#e2e8f0"}}>{s.userName}</strong> — {s.cycleName} · {new Date(s.dateTime).toLocaleString()}</div>)}</div></div>}
      </>}

      {/* USERS */}
      {tab==="users"&&<div className="C">
        <span style={{fontSize:15,fontWeight:700}}>Users</span>
        <div style={{display:"flex",gap:5,marginTop:12,marginBottom:12,flexWrap:"wrap"}}><input value={nn} onChange={e=>sNN(e.target.value)} placeholder="Name" className="I" style={{flex:2,minWidth:90,fontSize:12}}/><input value={np} onChange={e=>sNP(e.target.value)} placeholder="PIN" maxLength={6} className="I" style={{flex:1,minWidth:60,fontSize:12}}/><button onClick={addU} className="B" style={{background:cfg.primaryColor,color:"#fff"}}>Add</button></div>
        {users.map(u=><div key={u.id} className="sb" style={{padding:"9px 10px",borderRadius:9,background:u.disabled?"#dc262608":"#0c1220",marginBottom:4,border:u.disabled?"1px solid #dc262620":"1px solid transparent"}}>
          <div className="row"><div className="A" style={{width:30,height:30,fontSize:14,background:`${cfg.primaryColor}18`}}>{u.emoji||"😊"}</div><div><div style={{fontWeight:600,fontSize:12}}>{u.name}{u.disabled&&<span style={{color:"#f87171",fontSize:9,marginLeft:4}}>disabled</span>}</div><div className="M" style={{fontSize:9,color:"#64748b"}}>{u.pin} · {u.dnd?"DND":"ON"}</div></div></div>
          <div className="row" style={{gap:3}}><button onClick={()=>sEU(u)} className="B" style={{background:`${cfg.primaryColor}18`,color:cfg.primaryColor,fontSize:9,padding:"3px 7px"}}>Edit</button><button onClick={()=>{DB.removeUser(u.id);toast("Removed","info")}} className="B" style={{background:"#dc262618",color:"#ef4444",fontSize:9,padding:"3px 7px"}}>Del</button></div>
        </div>)}
      </div>}

      {/* CONTROL */}
      {tab==="ctrl"&&<div className="C">
        <span style={{fontSize:15,fontWeight:700}}>Control</span>
        <div style={{padding:12,borderRadius:9,background:"#0c1220",margin:"12px 0",border:`1px solid ${mac?.running?"#dc262630":"#22c55e30"}`}}>
          <div style={{fontSize:12,fontWeight:700}}>{mac?.running?`Running — ${mac.userName} (${mac.cycleName}) — ${rmM}m left`:"Idle"}</div>
        ...
        <button onClick={()=>sSFU(true)} className="B" style={{width:"100%",background:`${cfg.primaryColor}18`,color:cfg.primaryColor,border:`1px solid ${cfg.primaryColor}33`,marginBottom:12}}>Start wash for a user...</button>
        <span style={{fontSize:12,fontWeight:600}}>Manual relay test</span>
        <div className="G2" style={{marginTop:6,marginBottom:12}}>
          <button onClick={()=>{DB.setMachine({running:true,userId:"admin",userName:"Admin",cycleName:"Manual",startTime:Date.now(),durationMs:300000});toast("ON 5m","success")}} className="B" style={{background:"#059669",color:"#fff"}}>ON (5m)</button>
          <button onClick={()=>{DB.setMachine({running:false});toast("OFF","info")}} className="B" style={{background:"#dc2626",color:"#fff"}}>OFF</button>
        </div>
        <span style={{fontSize:12,fontWeight:600}}>Quick toggle per user</span>
        <div style={{marginTop:6}}>{users.filter(x=>!x.disabled).map(u=><div key={u.id} className="sb" style={{padding:"7px 8px",borderRadius:7,background:"#0c1220",marginBottom:3}}>
          <div className="row"><span style={{fontSize:12}}>{u.emoji||"😊"}</span><span style={{fontSize:11}}>{u.name}</span></div>
          <div className="row" style={{gap:3}}>
            {(!mac?.running||mac.userId!==u.id)?<button onClick={()=>{if(mac?.running)return toast("Busy","error");DB.setMachine({running:true,userId:u.id,userName:u.name,cycleName:"Admin",startTime:Date.now(),durationMs:45*60000});toast(`ON for ${u.name}`,"success")}} className="B" style={{background:"#05966918",color:"#34d399",fontSize:9,padding:"3px 8px"}}>Start</button>:null}
        ...
        {sch.length>0&&<><div style={{marginTop:12,fontSize:12,fontWeight:600}}>Scheduled ({sch.length})</div>{sch.map(s=><div key={s.id} style={{padding:"4px 7px",borderRadius:5,background:"#0c1220",marginBottom:2,fontSize:10}}>{s.userName} — {s.cycleName} · {new Date(s.dateTime).toLocaleString()}</div>)}<button onClick={()=>{DB.clearSchedule();toast("Cleared","info")}} className="B" style={{marginTop:4,background:"#dc262618",color:"#ef4444",fontSize:10}}>Clear all</button></>}
      </div>}

      {/* HISTORY */}
      {tab==="log"&&<div className="C">
        <span style={{fontSize:15,fontWeight:700}}>History ({hist.length})</span>
      ...
      {tab==="cfg"&&<div className="C">
        ...
        <div style={{display:"grid",gap:8,marginTop:12}}>
          ...
          <div><label style={{fontSize:10,color:"#94a3b8",marginBottom:3,display:"block"}}>ESP32 IP</label><input value={ip} onChange={e=>sIp(e.target.value)} className="I" style={{fontSize:12}} placeholder="Optional"/></div>
